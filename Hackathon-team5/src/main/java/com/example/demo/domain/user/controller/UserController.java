...
public class UserController {
    ...
    @Operation(summary = "현재 위치 업데이트", description = "사용자의 현재 위치(위도, 경도)를 업데이트합니다")
    @SwaggerConfig.ApiErrorExamples({
            ErrorCode.USER_NOT_FOUND,
            ErrorCode.INVALID_INPUT_VALUE,
            ErrorCode.AUTHENTICATION_REQUIRED
    })
    public ApiResponse<UpdateLocationResponseDto> updateLocation(
            @Parameter(hidden = true) @AuthUser Long userId,
            @Valid @RequestBody UpdateLocationRequestDto request) {

        UpdateLocationResponseDto response = userService.updateLocation(userId, request);
        return ApiResponse.success("위치 업데이트 성공", response);
    }
}
